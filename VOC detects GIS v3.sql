...
declare @facility_id int =   47 --NB
declare @loc_grp varchar (1000) = null
declare @mth_grp as varchar (200) = 'PGE gw VOCs'
declare @task_code varchar (1000) =   'row_GW_2016_3Q' -- 'BS_ROW_GW_2016_2Q' -- 'ROW_GW_2016_2Q' -
declare @param varchar (1000) = null
declare @coord_type varchar (50) = 'N83SPCA III Ft' --'SPCS27 CA III' --'N83SPCA III Ft' --'SPCS27 CA III' -- 'N83SPCA V' --
declare @chem_len int    --longest chemical in chem list 
declare @units varchar (20) = 'ug/l'
declare @screening_level varchar (200) = 'pge-sl-wg-201602'
...
if object_id('tempdb..#R1') is not null drop table #r1
	...
	insert into #R1
		...
		,task.task_ID
		,row_number() over(partition by sys_sample_code, sys_loc_code,sample_type_code,task_id order by sys_sample_code,sys_loc_Code,sample_type_code,chemical_name) as Row_ID
		,r.chemical_name
		,r.cas_rn
...
set @chem_count = 
	...
	select sys_sample_code, sys_loc_code,task_code, sample_type_code, count(chemical_name) chem_count
	from #r1 
	group by sys_sample_code,sys_loc_code, task_code, sample_type_code)z)
	...
	'max(cast(converted_result_unit as varchar)) as Units,' 
