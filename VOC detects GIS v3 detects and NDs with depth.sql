...
if object_id('tempdb..#R1') is not null drop table #r1
	...
	create table #r1 (facility_id int, mth_grp_name varchar(100), task_code varchar (30), sys_sample_code varchar (40), sys_loc_code varchar (20), sample_type_code varchar (10), sample_depth varchar(20), sample_date varchar (20), x_coord  float , y_coord  float
	...
	insert into #R1
		...
		,mg.grp_name
		,r.task_code
		,NULL --r.sys_sample_code
		,sys_loc_code
		,sample_type_code
		,case 
			when r.start_depth is not null and r.end_depth is not null 
			  then  cast(r.start_depth as varchar) + '-' + coalesce(cast(r.end_depth as varchar),'') --+ ' (' + depth_unit + ')'
			when r.start_depth is not null and r.end_depth is null 
			  then cast(r.start_depth as varchar) --+ ' (' + depth_unit + ')'
			when r.start_depth is null and r.end_depth is not null 
			  then cast(r.end_depth as varchar) --+ ' (' + depth_unit + ')'	
		end as sample_depth
...
set @depth_count = 
	(select max(depth_count) from (
	select sys_sample_code, sys_loc_code,task_code, sample_type_code, sample_date, count(sample_depth) depth_count
	from #r1 
	group by sys_sample_code,sys_loc_code, task_code, sample_type_code, sample_date)z)
